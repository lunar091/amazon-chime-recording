...
const s3_url:any = 'https://uat.yuinit.com/'
...
export class DemoMeetingApp
  ...
  buttonStates: { [key: string]: boolean } = {
    ...
    'button-live-transcription': true,
    'button-video-stats': false,
    'button-video-filter': false,
    'button-record-self': true,
  ...
  initEventListeners(): void {
    ...
    redirectHome.addEventListener('click', _e => {
    ...
    buttonMeetingEnd.addEventListener('click', _e => {
      ...
      AsyncScheduler.nextTick(async () => {
        ...
        if(this.type =='Interviewer'){
         
          var url =  s3_url + 'assignment-detail/interview/' + meetingId + '?end=' +   true
        await this.gotToJoinPage(url)
           
           // window.alert(meetingId)
           }else{
           var url  = s3_url + 'join/' + meetingId + '/' + attendeeId 
            await this.gotToJoinPage(url) 
  ...
  async joinMeeting(): Promise<any> {
    ...
    this.meeting = new URL(window.location.href).searchParams.get('m');
    this.accessToken = new URL(window.location.href).searchParams.get('accessToken');
    this.type = new URL(window.location.href).searchParams.get('type');
    this.toggleButton('button-record-self','on')
   
    this.return_url = new URL(window.location.href).searchParams.get('return_url');
    if(this.return_url){
      localStorage.setItem("return_url", this.return_url);
    }
    ...
    if(!this.meeting){
     ...
     if(meetingId && attendeeId){
       if(this.type =='Interviewer'){
        ...
        window.location.href=return_url + '/assignment-detail/interview/' + meetingId + '?end=' +   true
       
       }else{
         console.log("this.return_url2",this.return_url)
        window.location.href=return_url + '/join/' + meetingId + '/' + attendeeId  
      ...
      document.getElementById('meeting-error').innerText = 'Meeting not found'
    ...
    var record = new URL(window.location.href).searchParams.get('record')
    
    this.title = new URL(window.location.href).searchParams.get('title');
    this.scheduledStart = new URL(window.location.href).searchParams.get('scheduledStart');

    this.scheduledEnd = new URL(window.location.href).searchParams.get('scheduledEnd');
    var attendeeId = new URL(window.location.href).searchParams.get('attendeeId');
    this.internalMeetingId = new URL(window.location.href).searchParams.get('internalMeetingId');
   

    if(this.type=='Interviewee' || this.type=='Guest' || this.type=='Organization'){
      document.getElementById('button-meeting-end').style.display="none";
    ...
    if(record){
      ...
      response = await fetch(
        `${DemoMeetingApp.BASE_URL}join?meetingId=${encodeURIComponent(this.meeting)}&name=${encodeURIComponent(this.name)}` + `&internalMeetingId=${encodeURIComponent(this.internalMeetingId)}` + `&record=true` + `&accessToken=${encodeURIComponent(this.accessToken)}`,
        {
          method: 'POST',
        }
      );
      ...
      response = await fetch(
        `${DemoMeetingApp.BASE_URL}join?meetingId=${encodeURIComponent(this.meeting)}&name=${encodeURIComponent(this.name)}&region=${encodeURIComponent(this.region)}&accessToken=${encodeURIComponent(this.accessToken)}&title=${encodeURIComponent(this.title)}` + 
        `&scheduledStart=${encodeURIComponent(this.scheduledStart)}&scheduledEnd=${encodeURIComponent(this.scheduledEnd)}&attendeeId=${encodeURIComponent(attendeeId)}&type=${encodeURIComponent(this.type)}&internalMeetingId=${encodeURIComponent(this.internalMeetingId)}`
      ...
      if (json.error) {
        ...
        document.getElementById('meeting-error').innerText = json.error
    ...
    if(record){
      var url = '?record=true&interview/'  + this.meeting 
      document.getElementById('header-container-record').style.display="none";
     
    }else{
      var url = 'interview/'  + this.meeting 
    }
    ...
    localStorage.setItem("meetingId", this.meeting);
    localStorage.setItem("attendeeId", attendeeId);
    
  //  window.location.hash = this.meeting + '/' + attendeeId  
    return json;
  }
  ...
  async endMeeting(): Promise<any> {
    await fetch(`${DemoMeetingApp.BASE_URL}end?internalMeetingId=${encodeURIComponent(this.internalMeetingId)}`, {
      method: 'POST',
    });
  ...
  async gotToJoinPage(url): Promise<any> {
      ...
      this.audioVideo.setDeviceLabelTrigger(
        async (): Promise<MediaStream> => {
          ...
          try {
            ...
            (document.getElementById(
              'video-preview-error'
            ) as HTMLSpanElement).innerText = `Yuinit requires access to your camera and microphone. Click the camera blocked icon in your browser's address bar. `;
            (document.getElementById('joinButton') as HTMLInputElement).disabled = true;
            
            //console.log("sdsaa",err)
  ...
  async authenticate(): Promise<string> {
   ...
   // history.replaceState({}, `${this.meeting}`, url.toString());
    this.internalMeetingId = configuration.meetingId
    return configuration.meetingId;
  }


  
  async initMeeting(): Promise<string> {
   ...
   // this.showProgress('progress-authenticate');
    ...
    (document.getElementById(
      'meeting-id'
    ) as HTMLSpanElement).innerText = `${this.title}`;
    // (document.getElementById(
    //   'chime-meeting-id'
    // ) as HTMLSpanElement).innerText = `Meeting ID: ${chimeMeetingId}`;
    (document.getElementById(
    ...
    if(this.isRecorder()){
      console.log("isRecorder",this.isRecorder())
      this.name = this.isRecorder() ? '«Meeting Recorder»' : '«Meeting Broadcaster»';
      await this.openAudioOutputFromSelection();
        await this.join();
        this.displayButtonStates();
